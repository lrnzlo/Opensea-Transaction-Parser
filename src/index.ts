...
const Provider = new InfuraProvider('mainnet', process.env.InfuraProjectID);
 ...
 * @returns 
 * 
 * Here Etherscan Api returns ABI for contract.
 * Only verified contract can be load.
 * If you get problem loading ABI please check GetTransactionData() function to provide your own ABI
 */
async function GetABI(EtherscanAPIKEY: string, ContractAddress: string) {
  try {
    ...
    console.error(error, 'Something went wrong getting ABI from Etherscan\nCheck if the contract is verified.\nEtherscan API can load only ABI from verified contract source code.');
  }
}
/**
 * 
 * @param TrxHash 
 * @param EtherscanAPIKEY 
 * @param Abi If the contract you try to load from Etherscan API is not verified please provide the abi as string 
 * @returns
 * 
 */
...
export async function GetTransactionData(
  ...
  try {
    ...
    for (const _ of t_logs) {
      const abi = Abi ?  Abi : await GetABI(EtherscanAPIKEY, _.address);
...
export async function DecodeTransactionInfo(
  ...
  const ContractsERC721 = new Contract(logs[2].address, ERC721Abi, Provider);

  const TransactionInfo = logs[0].iface.parseLog({
    data: logs[0].data,
    topics: logs[0].topics,
  });
  const RoyaltyInfo = logs[1].iface.parseLog({
    data: logs[1].data,
    topics: logs[1].topics,
  });
  const NftInfo = logs[2].iface.parseLog({
    data: logs[2].data,
    topics: logs[2].topics,
  });
  const NftCollectionName = await ContractsERC721.name();

  const results: OpenseaTransactionInfo = {
    Seller: TransactionInfo.args['dst'],
    Buyer: TransactionInfo.args['src'],

    NftId: NftInfo.args['tokenId'].toString(),
    NftCollection: { Address: logs[2].address },

    Price: ETH(TransactionInfo.args['wad'].toString()) + 'ETH',
    Royalty: {
      val: ETH(RoyaltyInfo.args['wad'].toString()) + 'ETH',
      collector: RoyaltyInfo.args['dst'],
    },
  };

  NftCollectionName != ''
    ? (results.NftCollection.Name = NftCollectionName)
    : null;
  return results;
}
